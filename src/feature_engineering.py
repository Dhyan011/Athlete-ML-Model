...
def extract_features(data_dir='dataset(31)', is_train=True):
    """
    Extracts tabular features from 30-day wearable telemetry and athlete metadata.
    Ensures strict observation window constraints (days 1-30 only).
    """
    ...
    for w in [3, 5, 10]:
        w_dates = obs_dates[-w:]
        w_df = daily_obs[daily_obs['ActivityDate'].isin(w_dates)]
        steps_w = w_df.groupby('Id')['TotalSteps'].mean().reset_index()
        steps_w.columns = ['athlete_id', f'acute_{w}d_steps']
        cal_w = w_df.groupby('Id')['Calories'].mean().reset_index()
        cal_w.columns = ['athlete_id', f'acute_{w}d_cal']
        vam_w = w_df.groupby('Id')['VeryActiveMinutes'].mean().reset_index()
        vam_w.columns = ['athlete_id', f'acute_{w}d_vam']
        daily_feats = daily_feats.merge(steps_w, on='athlete_id', how='left')
        daily_feats = daily_feats.merge(cal_w, on='athlete_id', how='left')
        daily_feats = daily_feats.merge(vam_w, on='athlete_id', how='left')
        daily_feats[f'acwr_steps_{w}d'] = daily_feats[f'acute_{w}d_steps'] / (daily_feats['daily_TotalSteps_mean'] + 1e-5)
        daily_feats[f'acwr_cal_{w}d'] = daily_feats[f'acute_{w}d_cal'] / (daily_feats['daily_Calories_mean'] + 1e-5)
        daily_feats[f'acwr_vam_{w}d'] = daily_feats[f'acute_{w}d_vam'] / (daily_feats['daily_VeryActiveMinutes_mean'] + 1e-5)
        daily_feats.drop(columns=[f'acute_{w}d_steps', f'acute_{w}d_cal', f'acute_{w}d_vam'], inplace=True)

    w3_df = daily_obs[daily_obs['ActivityDate'].isin(week3_dates)]
    w3_steps = w3_df.groupby('Id')['TotalSteps'].mean().reset_index().rename(columns={'Id': 'athlete_id', 'TotalSteps': 'w3_steps_mean'})
    daily_feats = daily_feats.merge(w3_steps, on='athlete_id', how='left')
    daily_feats['acwr_trend_w4_w3'] = (daily_feats['w4_TotalSteps_mean'] - daily_feats['w3_steps_mean']) / (daily_feats['w3_steps_mean'] + 1e-5)
    daily_feats.drop(columns=['w3_steps_mean'], inplace=True)

    last3_df = daily_obs[daily_obs['ActivityDate'].isin(obs_dates[-3:])]
    last3_spike = last3_df.groupby('Id')['TotalSteps'].max().reset_index().rename(columns={'Id': 'athlete_id', 'TotalSteps': 'last3_step_spike'})
    daily_feats = daily_feats.merge(last3_spike, on='athlete_id', how='left')
    daily_feats['last3_step_spike_ratio'] = daily_feats['last3_step_spike'] / (daily_feats['daily_TotalSteps_mean'] + 1e-5)
    daily_feats.drop(columns=['last3_step_spike'], inplace=True)
...
if __name__ == '__main__':
    ...
    print(f"Features extracted: shape {df.shape} -> saved to processed_data/master_features.csv")
